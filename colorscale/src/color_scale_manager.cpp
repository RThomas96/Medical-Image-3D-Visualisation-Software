...
namespace Color {
	...
	void ColorScaleManager::init_default_color_scales() {
		...
		this->hsv2rgb	= std::make_shared<ColorScaleFunctor>([](float f) -> glm::vec3 {
			  ...
			  return glm::vec3(glm::mix(glm::vec3(ScrollingRGB), glm::mix(glm::vec3(ScrollingRGB.z, ScrollingRGB.x, ScrollingRGB.y), glm::vec3(ScrollingRGB.y, ScrollingRGB.z, ScrollingRGB.x), IsNotSecondSlice), IsNotFirstSlice));	 // Make the RGB rotate right depending on final slice index
		  });
	...
	void ColorScaleManager::removeColorScale(ColorScaleBase::Ptr to_remove) {
		...
		if (to_remove == this->greyscale || to_remove == this->hsv2rgb) {
			return;
		}
