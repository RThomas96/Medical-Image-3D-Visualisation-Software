...
GridView::GridView(GridDetailedView* const _details, const std::shared_ptr<DiscreteGrid>& _grid) :
	grid(_grid) {
	if (_details == nullptr) {
		throw std::runtime_error("[ERROR] Details was null.\n");
	}

	this->detailedView = _details;
	if (_grid.get() == nullptr) {
		throw std::runtime_error("[ERROR] Grid was null.\n");
	}
