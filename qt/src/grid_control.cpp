...
GridControl::GridControl(std::shared_ptr<DiscreteGrid> _vg, InterpolationMesh::Ptr _tm, Scene* _scene, QWidget* parent) :
	QWidget(parent) {
	this->voxelGrid = _vg;
	this->mesh		= _tm;
	this->scene		= _scene;
...
void GridControl::setupWidgets() {
	...
	QLabel* label_gridControl = new QLabel("Grid Control");
	this->strayObj.push_back(label_gridControl);
	QLabel* label_gridSizeHeader = new QLabel("# of voxels");
	this->strayObj.push_back(label_gridSizeHeader);
	QLabel* label_voxelSizeHeader = new QLabel("Voxel sizes");
	this->strayObj.push_back(label_voxelSizeHeader);
	QLabel* label_Axis_X = new QLabel("X : ");
	this->strayObj.push_back(label_Axis_X);
	QLabel* label_Axis_Y = new QLabel("Y : ");
	this->strayObj.push_back(label_Axis_Y);
	QLabel* label_Axis_Z = new QLabel("Z : ");
	this->strayObj.push_back(label_Axis_Z);
	...
	QLabel* label_InterpolationMethod = new QLabel("Interpolation method :");
	this->strayObj.push_back(label_InterpolationMethod);
	...
	// Add voxel sizes :
	mainLayout->addWidget(frame_VoxelSizes, mRow, 0, 1, -1);
	mRow += 2;	  // space to next widget
	// Add bb controls :
	mainLayout->addWidget(frame_BoundingBox, mRow, 0, 1, -1);
	mRow += 2;	  // space to next widget
	// group box for the output selector
	mainLayout->addWidget(this->groupBox_outputType, mRow, 0, 1, -1);
	mRow += 2;
	// Add save options :
	mainLayout->addLayout(layout_saveFile, mRow, 0, 1, -1);
	mRow += 2;	  // space to next widget
	// Add buttons :
...
void GridControl::setGridResolution() {
	...
	if (this->output_R) {
		this->output_R->setResolution(userRes);
	}
	if (this->output_B) {
		this->output_B->setResolution(userRes);
	}
	if (this->output_RGB) {
		this->output_RGB->setResolution(userRes);
	}
...
void GridControl::setGridBoundingBox() {
	...
	if (this->output_R) {
		this->output_R->setBoundingBox(userBBox);
	}
	if (this->output_B) {
		this->output_B->setBoundingBox(userBBox);
	}
	if (this->output_RGB) {
		this->output_RGB->setBoundingBox(userBBox);
	}
...
void GridControl::setGridVoxelSize() {
	...
	if (this->output_R) {
		this->output_R->setVoxelDimensions(userVxDims);
	}
	if (this->output_B) {
		this->output_B->setVoxelDimensions(userVxDims);
	}
	if (this->output_RGB) {
		this->output_RGB->setVoxelDimensions(userVxDims);
	}
