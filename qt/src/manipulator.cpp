...
namespace UITool {
    ...
    MeshManipulator::MeshManipulator(int nbManipulators):nbManipulators(nbManipulators), manipulators(std::vector<VertexManipulator>(nbManipulators)) {
        ...
        for(int i = 0; i < nbManipulators; ++i) {
            this->manipulators[i].setAssignedIdx(i);
            this->lock(i);
        }
    }
