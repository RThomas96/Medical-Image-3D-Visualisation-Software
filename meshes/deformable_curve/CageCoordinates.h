	...
	void computeCoordinatesOf3dPoint(
	  ...
	  std::vector<double>& _FC_psi) {
		...
		for (unsigned int t = 0; t < cage_triangles.size(); ++t)
		{
			...
			double signed_solid_angle = __signed_solid_angle(e_normalized[0],
										  e_normalized[1],
										  e_normalized[2]) /
										(4.f * M_PI);
			...
			double At = point_t::crossProduct(
						  ...
						  cage_vertices[tri[2]] - cage_vertices[tri[0]])
						  .norm() /
						2.f;
	...
	void computeCoordinatesOf3dPoint(
	  ...
	  std::vector<point_t>& _FC_psi_gradient) {
		...
		for (unsigned int t = 0; t < cage_triangles.size(); ++t)
		{
			...
			float signed_solid_angle = __signed_solid_angle(e_normalized[0],
										 e_normalized[1],
										 e_normalized[2]) /
									   (4.f * M_PI);
...
namespace GreenCoords {
	...
	public:
		GreenScalingFactor(normal_t const& u, normal_t const& v) :
			_u(u), _v(v), _s(1.f) {}
	...
	float GCTriInt(point_t const& p, point_t const& v1, point_t const& v2, point_t const& eta) {
		...
		for (int i = 0; i < 2; ++i)
		{
			...
			I_theta[i]			  = -sign_S_div_by_2 *
						 (2.f * sqrt_c * std::atan(sqrt_c * C / std::sqrt(lambda + S * S * c)) + sqrt_lambda * std::log((2.f * sqrt_lambda * S * S / ((1.f - C) * (1.f - C))) * (1.f - 2.f * c * C / (c * (1.f + C) + lambda + std::sqrt(lambda * lambda + lambda * c * S * S)))));
