...
\chapter{État de l'art}\label{chapter:02:stateoftheart}
{
	...
	\commentaire{Pour contenu, cf le rapport biblio, reprendre les grandes lignes}\par
	\commentaire{Note : on peut inverser les infos sur la volumique, et sur la prostate/Gleason, ce qui est le plus fluide.}\par
		...
		\subsection{Reconstructions volumiques}
		{
			...
			\subsubsection{Interpolation et rééchantillonnage}
			{
				...
				\paragraph*{Interpolation au plus proche voisin}
				{
					...
					\commentaire{Ajouter définition de f, et labelliser l'equation et mettre graphique pour montrer ce que ca représente en 1D}\par
				...
				{
					\wip{Insérer infos interpolation barycentriques avec graphique 2D}
				}
			}
		}
		% }}}
	...
	{
		\commentaire{à dire : ils utilisent des outils non adaptés à la tâche (plugins ImageJ et code matlab non adapté), ne prennent pas en compte les spécificités du microscope qu'ils ont, et ensuite ils font meme pas les opérations prévues car prends trop de temps.}\\
	}
