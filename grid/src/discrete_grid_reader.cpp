...
namespace IO {
	...
	GenericGridReader::GenericGridReader(data_t _thres) :
		threshold(_thres) {
	...
	GenericGridReader& GenericGridReader::parseImageInfo(ThreadedTask::Ptr& task) {
		task->setSteps(0);
		return *this;
	}
	...
	GenericGridReader& GenericGridReader::loadImage(ThreadedTask::Ptr& task) {
		task->setSteps(0);
		return *this;
	}
	...
	DIMReader::DIMReader(data_t thresh) :
		GenericGridReader(thresh) {
		this->dimFile = nullptr;
		this->imaFile = nullptr;
	}

	DIMReader::~DIMReader() {
		if (this->dimFile != nullptr) {
			this->dimFile->close();
		}
	...
	DIMReader& DIMReader::parseImageInfo(ThreadedTask::Ptr& task) {
		...
		do {
			...
			if (token.find("-type") != std::string::npos) {
				(*this->dimFile) >> type;
			} else if (token.find("-dx") != std::string::npos) {
				(*this->dimFile) >> this->voxelDimensions.x;
			} else if (token.find("-dy") != std::string::npos) {
				(*this->dimFile) >> this->voxelDimensions.y;
			} else if (token.find("-dz") != std::string::npos) {
				(*this->dimFile) >> this->voxelDimensions.z;
			} else {
	...
	DIMReader& DIMReader::loadImage(ThreadedTask::Ptr& task) {
		...
		if (this->downsampleLevel == DownsamplingLevel::Low) {
			dataDims /= std::size_t(2);
			slicesToLoad = 2;
		}
		if (this->downsampleLevel == DownsamplingLevel::Lower) {
			dataDims /= std::size_t(4);
			slicesToLoad = 4;
		}
		if (this->downsampleLevel == DownsamplingLevel::Lowest) {
			dataDims /= std::size_t(8);
			slicesToLoad = 8;
		}
		...
		std::cerr << "\tNew      vox  dimensions : {" << this->voxelDimensions.x * this->voxelMultiplier.x << ',' << this->voxelDimensions.y * this->voxelMultiplier.y << ',' << this->voxelDimensions.z * this->voxelMultiplier.z << "}\n";
	...
	StackedTIFFReader::StackedTIFFReader(StackedTIFFReader::data_t thresh) :
		GenericGridReader(thresh) {
		...
		for (std::size_t i = 0; i < this->filenames.size(); ++i) {
			...
			do {
				if (TinyTIFFReader_wasError(this->tiffFile)) {
					std::cerr << "[WARNING] Reading of TIFF frames for filename " << this->filenames[i] << " caused an error.\n";
					std::cerr << "[WARNING] Error message : " << TinyTIFFReader_getLastError(this->tiffFile) << '\n';
				}
	...
	libTIFFReader::TIFFFrame::TIFFFrame(std::string _filename, tdir_t idx) :
		TIFFFrame() {
	...
	void libTIFFReader::TIFFFrame::loadTIFFInfo(tdir_t index) {
		...
		if (this->filename.empty() == false) {
			...
			if (result != 1) {
				throw std::runtime_error("cannot set file to directory " + std::to_string(index));
			}
			...
			if (this->samplesPerPixel > 3u) {
				throw std::runtime_error("cant parse images with more than 3 samples.");
			}
			...
			if (result != 1) {
				throw std::runtime_error("cannot read height field in this directory");
			}
	...
	libTIFFReader::libTIFFReader(data_t thresh) :
		GenericGridReader(thresh) {
		this->frames.clear();
	...
	libTIFFReader& libTIFFReader::parseImageInfo(ThreadedTask::Ptr& task) noexcept(false) {
		...
		std::for_each(this->frames.cbegin(), this->frames.cend(), [this, &task](const TIFFFrame& f) {
			if (f.width != this->imageDimensions.x || f.height != this->imageDimensions.y) {
				std::cerr << "[ERROR_DIMENSIONS] Frame " << f.directoryOffset << " of file " << f.filename << " is not of the same dimensions. fDims = [" << f.width << ", " << f.height << "] compared to [" << this->imageDimensions.x << ", " << this->imageDimensions.y << "]\n";
			}
			task->advance();
		});
	...
	libTIFFReader& libTIFFReader::preAllocateStorage() {
		...
		if (this->downsampleLevel == IO::DownsamplingLevel::Low) {
			divider = 2u;
		}
		if (this->downsampleLevel == IO::DownsamplingLevel::Lower) {
			divider = 4u;
		}
		if (this->downsampleLevel == IO::DownsamplingLevel::Lowest) {
			divider = 8u;
		}
	...
	libTIFFReader& libTIFFReader::openFile(const std::string& filename) {
		...
		if (nod == 0) {
			throw std::runtime_error("File " + filename + " had no directories !");
		}
	...
	libTIFFReader& libTIFFReader::loadSlice(std::size_t idx, std::vector<data_t>& tgt) {
		...
		if (result != 1) {
			throw std::runtime_error("Could not set the TIFF directory to" + std::to_string(frame.directoryOffset));
		}

		// Compute offset for the pixels :
		uint64_t pixelOffset = 0;
		for (uint16_t px = 0; px < frame.samplesPerPixel; ++px) {
			pixelOffset += frame.bitsPerSample[px];
		}
	...
	OMETIFFReader::OMETIFFReader(data_t thresh) :
		libTIFFReader(thresh) {}
	...
	OMETIFFReader& OMETIFFReader::parseImageInfo(ThreadedTask::Ptr& task) {
		...
		if (strlen(desc) == 0) {
			throw std::runtime_error("Error : TIFF description did not contain any data");
		}
				...
				if (reader.name().compare(QString("TiffData"), Qt::CaseSensitivity::CaseSensitive) == 0) {
					...
					if ((filenameRef = reader.attributes().value("FileName")).isNull() == false) {
						...
						if (QFileInfo::exists(defaultDir.path() + "/" + filename)) {
							...
							std::cerr << "[Error] File " << filename.toStdString() << " does not exist in " << defaultDir.path().toStdString() << '\n';
						}
					}
