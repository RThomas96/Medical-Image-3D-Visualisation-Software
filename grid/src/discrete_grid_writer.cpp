#include "../include/discrete_grid_writer.hpp"
...
namespace IO {
	...
	GenericGridWriter& GenericGridWriter::setBaseName(std::string bname) {
		this->baseName = bname;
		return *this;
	}
	GenericGridWriter& GenericGridWriter::setBasePath(std::string bpath) {
		this->basePath = bpath;
		return *this;
	}
	GenericGridWriter& GenericGridWriter::setGrid(const std::shared_ptr<DiscreteGrid>& g) {
		this->grid = g;
		return *this;
	}
...
#warning TinyTIFFWriter has not been tested here !
	...
	StaggeredTIFFWriter::StaggeredTIFFWriter(const std::string _bN, const std::string _bP) :
		GenericGridWriter(_bN, _bP) {
		this->tiffFile = nullptr;
	}
	...
	StaggeredTIFFWriter::~StaggeredTIFFWriter(void) {
		if (this->tiffFile != nullptr) {
			...
			if (TinyTIFFWriter_wasError(this->tiffFile)) {
				std::cerr << "[ERROR] : "
						  << "Could not close file in StackedTIFFWriter.\n";
			}
			this->tiffFile = nullptr;
		}
	...
	std::string StaggeredTIFFWriter::createSuffixedFilename(std::size_t idx) {
		...
		for (; completeLength > currentLength; --completeLength) {
			suffixed += "0";
		}
