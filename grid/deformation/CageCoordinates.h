...
namespace URAGO
{
    ...
    double __signed_solid_angle(point_t const & a, point_t const & b, point_t const & c)
    {
        ...
        double div = al*bl*cl + dot(a,b)*cl + dot(a,c)*bl + dot(b,c)*al;
            ...
            std::vector< double > & _FC_psi)
    {
        ...
        for( unsigned int t = 0 ; t < cage_triangles.size() ; ++t )
        {
            ...
            double signed_volume = dot( cross(e[0],e[1]) , e[2] ) / 6.f;
            ...
            for( unsigned int v = 0 ; v < 3 ; ++v )
            {
                J[v] = cross( e[(v+2)%3] , e[(v+1)%3] );
            }

            _FC_psi[t] = - 3.f * signed_solid_angle * signed_volume / At ;
            for( unsigned int v = 0 ; v < 3 ; ++v )
                _FC_psi[t] -= C[v]* dot(J[v],Nt);

            for( unsigned int v = 0 ; v < 3 ; ++v )
            {
                _VC_phi[ tri[v] ] += dot(Pt , J[v]) / (2.f * At);
            }
        }
    }
            ...
            std::vector< point_t > & _FC_psi_gradient )
    {
        ...
        for( unsigned int t = 0 ; t < cage_triangles.size() ; ++t )
        {
            ...
            float signed_volume = dot( cross(e[0],e[1]) , e[2] ) / 6.f;
            ...
            for( unsigned int v = 0 ; v < 3 ; ++v )
            {
                _FC_psi[t] -= C[v]* point_t::dot(J[v],Nt);
                _FC_psi_gradient[t] = -Pt;
            }

            for( unsigned int v = 0 ; v < 3 ; ++v )
            {
                _VC_phi[ tri[v] ] += point_t::dot(Pt , J[v]) / (2.f * At);
                _VC_phi_gradient[ tri[v] ] += point_t::cross(Pt,d[v]) / (2.f * At);
            }
...
namespace GreenCoords
{
    ...
    {
        float c_alpha = std::max( std::min( dot( v2 - v1 , p - v1 ) / ( (v2 - v1).norm() * (p - v1).norm() ) , 1.f ) , -1.f );
        float alpha = std::acos( c_alpha );
        float s_alpha = std::sin( alpha );

        float surfaceSqr = ( cross( v2 - v1 , p - v1 ) ).sqrnorm();
        ...
        float c_beta = std::max( std::min( dot( v1 - p , v2 - p ) / ( (v1 - p).norm() * (v2 - p).norm() ) , 1.f ) , -1.f );
        float beta = std::acos( c_beta );

        float lambda = ( p - v1 ).sqrnorm() * s_alpha * s_alpha;
        float sqrt_lambda = std::sqrt( lambda );

        float c = ( p - eta ).sqrnorm();
            ...
            std::vector< float > & _FC_psi)
    {
        ...
        */
        ...
        for( unsigned int fi = 0 ; fi < cage_triangles.size() ; ++fi )
        {
            ...
            if( w.sqrnorm() >= epsilonSqr )
            {
                for( int l = 0 ; l < 3 ; ++l )
                    _VC_phi[ v[l] ] += dot( N[ (l+1) % 3 ] , w ) / dot( N[ (l+1) % 3 ] , vj[l] );
            }
        }
    }
}
...
namespace MVCCoords
{
    ...
    void computeMVCCoordinatesOf3dPoint(
            ...
            std::vector< point_t > const & cage_vertices ,
            std::vector< point_t > const & cage_normals ,
            std::vector< std::pair< unsigned int , float > > & _phi)
    {
        assert( cage_normals.size() == cage_triangles.size()   &&    "cage_normals.size() != cage_triangles.size()" );
        ...
        for( unsigned int t = 0 ; t < cage_triangles.size() ; ++t )
        {
            ...
            {
                l[ i % 3 ] = ( u[ vid[ ( i + 1 ) % 3 ] ] - u[ vid[ ( i - 1 ) % 3 ] ] ).norm();
            }
