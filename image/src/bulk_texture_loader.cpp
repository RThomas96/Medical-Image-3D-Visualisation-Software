#include "../include/bulk_texture_loader.hpp"
...
#define CLAMP_VAL(x) (x>128)?255:x<<1
#define THRESHOLD 5
...
bulk_texture_loader::bulk_texture_loader() {
	...
	// No image was yet loaded, initialize the sizes to 0 :
	this->image_width = static_cast<std::size_t>(0);
	this->image_height = static_cast<std::size_t>(0);
...
void bulk_texture_loader::initialize_image_data(const QString& test_file, int nb_images) {
	...
	std::size_t size_to_allocate = this->downsampling_enabled ?
					       (this->image_width * this->image_height) / 4 * this->image_depth :
					       this->image_width * this->image_depth * this->image_height;

	std::cout << "Downsampling is " << (this->downsampling_enabled ? "enabled." : "disabled.") << std::endl;
	if (this->downsampling_enabled) {
		std::cout << "Will allocate " << size_to_allocate << " elements (" << (static_cast<double>(size_to_allocate)/1024.)/1024. <<
			     "Mo) instead of " << this->image_width * this->image_depth * this->image_height << " elements (" <<
			     (static_cast<double>(this->image_width * this->image_depth * this->image_height)/1024.)/1024.<< "Mo)." <<std::endl;
	} else {
		std::cout << "Will allocate " << size_to_allocate << " elements (" << (static_cast<double>(size_to_allocate)/1024.)/1024. << "Mo)" << std::endl;
	}
...
void bulk_texture_loader::load_all_images(QStringList absolute_paths) {
	// If no images are present, exit :
	if (absolute_paths.size() == 0) {
		std::cerr << "bulk_texture_loader::load_all_images() : no paths were provided" << std::endl;
		return;
	}
...
void bulk_texture_loader::load_single_image(QString absolute_path) {
	this->tiff_file = nullptr;
	...
	if (downsampling_enabled == false ) {
		for (std::size_t i = 0; i < stride; ++i) {
			this->image_data[this->end_iterator] = CLAMP_VAL(this->raw_data_storage[i]);
			this->end_iterator++;
		}
	} else {
		for (std::size_t i = 0; i < this->image_height/2; ++i) {
			for (std::size_t j = 0; j < this->image_width/2; ++j) {
				this->image_data[this->end_iterator] = CLAMP_VAL(this->raw_data_storage[i*2*this->image_width + j*2]);
				this->end_iterator++;
			}
		}
	}
...
std::size_t bulk_texture_loader::get_image_depth() const { return this->image_depth; }

std::size_t bulk_texture_loader::get_image_width() const { return not this->downsampling_enabled ? this->image_width : this->image_width/2; }

std::size_t bulk_texture_loader::get_image_height() const { return not this->downsampling_enabled ? this->image_height : this->image_height/2; }

void bulk_texture_loader::enable_downsampling(bool _is_enabled) { this->downsampling_enabled = _is_enabled; }
