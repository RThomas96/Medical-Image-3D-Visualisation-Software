...
namespace Image {
	...
	class GridSubRegionDetail : public GridSubregion {
		public:
			/// @b Public typedef to the internal pixel type.
			typedef element_t pixel_t;

			/// @b Pointer type to reference this backend implementation detail.
			typedef std::unique_ptr<GridSubRegionDetail<pixel_t>> Ptr;

			/// @b The type of a simple read cache, keeping the last few slices in memory.
			typedef ReadCache<std::size_t, std::vector<pixel_t>> cache_t;

		protected:
			/// @b Sets the right variables when creating a grid.
			GridSubRegionDetail(svec3 origin, svec3 size, Grid::Ptr parent_grid);

		protected:
			/// @b Loads a slice from the parent grid, and cache it for later use.
			std::size_t load_slice_from_parent_grid(std::size_t slice_idx);

			/// @b Single function to call when trying to read a sub-region of the parent grid.
			template <typename out_t>
			bool templated_read_region(svec3 read_origin, svec3 read_size, std::vector<out_t>& return_values);

			/// @b Single function to call when trying to read the sample value ranges of the parent grid.
		...
		protected:
			/// @b The simple read cache, keeping some slices in memory.
			cache_t read_cache;
	};
