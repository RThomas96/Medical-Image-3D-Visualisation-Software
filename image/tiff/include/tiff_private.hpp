...
namespace Tiff {
	...
	class TIFFPrivate {
		...
		public:
			...
			TIFFPrivate& appendFrame(std::size_t idx, Frame fr);

			/// @b Add a fully specified image to the mix
			TIFFPrivate& addImage(TIFFImage& img);

			/// @b Returns the min and max values in the image, or representable by the internal type.
			template <typename target_t> void getRangeValues(glm::vec<2, target_t, glm::defaultp>& _range);
