...
namespace Image {
	...
	namespace Tiff {
		...
		TIFFWriterTemplated<unsupported_element_t>::TIFFWriterTemplated(std::string bname) :
			TIFFWriter(bname) {
			throw std::runtime_error("Error : cannot create a TIFF writer backend with unsupported type.");
		}

		template <typename unsupported_element_t>
		TIFFWriterTemplated<unsupported_element_t>::TIFFWriterTemplated(std::string bname, std::string bpath) :
			TIFFWriter(bname, bpath) {
			throw std::runtime_error("Error : cannot create a TIFF writer backend with unsupported type.");
		}

		template <>
		TIFFWriterTemplated<std::uint8_t>::TIFFWriterTemplated(std::string bname) :
			TIFFWriter(bname) {
		...
		template <>
		TIFFWriterTemplated<std::uint16_t>::TIFFWriterTemplated(std::string bname) :
			TIFFWriter(bname) {
			this->sample_format	  = SAMPLEFORMAT_UINT;
		...
		template <>
		TIFFWriterTemplated<std::uint32_t>::TIFFWriterTemplated(std::string bname) :
			TIFFWriter(bname) {
			this->sample_format	  = SAMPLEFORMAT_UINT;
		...
		template <>
		TIFFWriterTemplated<std::uint64_t>::TIFFWriterTemplated(std::string bname) :
			TIFFWriter(bname) {
			this->sample_format	  = SAMPLEFORMAT_UINT;
		...
		template <>
		TIFFWriterTemplated<std::uint8_t>::TIFFWriterTemplated(std::string bname, std::string bpath) :
			TIFFWriter(bname, bpath) {
			this->sample_format	  = SAMPLEFORMAT_UINT;
			this->bits_per_sample = 8;
			this->planar_config	  = PLANARCONFIG_CONTIG;
		...
		template <>
		TIFFWriterTemplated<std::uint16_t>::TIFFWriterTemplated(std::string bname, std::string bpath) :
			TIFFWriter(bname, bpath) {
			this->sample_format	  = SAMPLEFORMAT_UINT;
			this->bits_per_sample = 16;
			this->planar_config	  = PLANARCONFIG_CONTIG;
		...
		template <>
		TIFFWriterTemplated<std::uint32_t>::TIFFWriterTemplated(std::string bname, std::string bpath) :
			TIFFWriter(bname, bpath) {
			this->sample_format	  = SAMPLEFORMAT_UINT;
			this->bits_per_sample = 32;
			this->planar_config	  = PLANARCONFIG_CONTIG;
		...
		template <>
		TIFFWriterTemplated<std::uint64_t>::TIFFWriterTemplated(std::string bname, std::string bpath) :
			TIFFWriter(bname, bpath) {
			this->sample_format	  = SAMPLEFORMAT_UINT;
			this->bits_per_sample = 64;
			this->planar_config	  = PLANARCONFIG_CONTIG;
		...
		template <>
		TIFFWriterTemplated<std::int8_t>::TIFFWriterTemplated(std::string bname) :
			TIFFWriter(bname) {
		...
		template <>
		TIFFWriterTemplated<std::int16_t>::TIFFWriterTemplated(std::string bname) :
			TIFFWriter(bname) {
			this->sample_format	  = SAMPLEFORMAT_INT;
			this->bits_per_sample = 16;
			this->planar_config	  = PLANARCONFIG_CONTIG;
		...
		template <>
		TIFFWriterTemplated<std::int32_t>::TIFFWriterTemplated(std::string bname) :
			TIFFWriter(bname) {
			this->sample_format	  = SAMPLEFORMAT_INT;
			this->bits_per_sample = 32;
			this->planar_config	  = PLANARCONFIG_CONTIG;
		...
		template <>
		TIFFWriterTemplated<std::int64_t>::TIFFWriterTemplated(std::string bname) :
			TIFFWriter(bname) {
			this->sample_format	  = SAMPLEFORMAT_INT;
			this->bits_per_sample = 64;
			this->planar_config	  = PLANARCONFIG_CONTIG;
		...
		template <>
		TIFFWriterTemplated<std::int8_t>::TIFFWriterTemplated(std::string bname, std::string bpath) :
			TIFFWriter(bname, bpath) {
			this->sample_format	  = SAMPLEFORMAT_INT;
			this->bits_per_sample = 8;
			this->planar_config	  = PLANARCONFIG_CONTIG;
		...
		template <>
		TIFFWriterTemplated<std::int16_t>::TIFFWriterTemplated(std::string bname, std::string bpath) :
			TIFFWriter(bname, bpath) {
			this->sample_format	  = SAMPLEFORMAT_INT;
			this->bits_per_sample = 16;
			this->planar_config	  = PLANARCONFIG_CONTIG;
		...
		template <>
		TIFFWriterTemplated<std::int32_t>::TIFFWriterTemplated(std::string bname, std::string bpath) :
			TIFFWriter(bname, bpath) {
			this->sample_format	  = SAMPLEFORMAT_INT;
			this->bits_per_sample = 32;
			this->planar_config	  = PLANARCONFIG_CONTIG;
		...
		template <>
		TIFFWriterTemplated<std::int64_t>::TIFFWriterTemplated(std::string bname, std::string bpath) :
			TIFFWriter(bname, bpath) {
			this->sample_format	  = SAMPLEFORMAT_INT;
			this->bits_per_sample = 64;
			this->planar_config	  = PLANARCONFIG_CONTIG;
		...
		template <>
		TIFFWriterTemplated<float>::TIFFWriterTemplated(std::string bname) :
			TIFFWriter(bname) {
		...
		template <>
		TIFFWriterTemplated<double>::TIFFWriterTemplated(std::string bname) :
			TIFFWriter(bname) {
			this->sample_format	  = SAMPLEFORMAT_IEEEFP;
			this->bits_per_sample = 64;
			this->planar_config	  = PLANARCONFIG_CONTIG;
		...
		template <>
		TIFFWriterTemplated<float>::TIFFWriterTemplated(std::string bname, std::string bpath) :
			TIFFWriter(bname, bpath) {
			this->sample_format	  = SAMPLEFORMAT_IEEEFP;
			this->bits_per_sample = 32;
			this->planar_config	  = PLANARCONFIG_CONTIG;
		...
		template <>
		TIFFWriterTemplated<double>::TIFFWriterTemplated(std::string bname, std::string bpath) :
			TIFFWriter(bname, bpath) {
			this->sample_format	  = SAMPLEFORMAT_IEEEFP;
			this->bits_per_sample = 64;
			this->planar_config	  = PLANARCONFIG_CONTIG;
		...
		TIFFWriterTemplated<element_t>::~TIFFWriterTemplated() {}

		template <typename element_t>
		bool TIFFWriterTemplated<element_t>::writeSlice(Grid::Ptr src_grid, std::size_t slice, ThreadedTask::Ptr task) {
			...
			if (slice >= resolution.z) {
				std::cerr << "Error : cannot write past-the-end of the grid.\n";
				return false;
			}
			...
			if (dimensionality == 0) {
				std::cerr << "Error : cannot write 0 samples.\n";
				return false;
			}
			...
			for (std::size_t channel = 0; channel < dimensionality; ++channel) {
				...
				if (strip_size == 0) {
					std::cerr << "Error : slice " << slice << ", channel " << channel << " of file \"" << target_name << "\" cannot be written to (no strip size).";
		...
		template <typename element_t>
		bool TIFFWriterTemplated<element_t>::writeGrid(Grid::Ptr src_grid, ThreadedTask::Ptr task) {
			if (src_grid == nullptr) {
				return false;
			}
		...
		template <typename element_t>
		std::uint32_t TIFFWriterTemplated<element_t>::write_tiff_tags(TIFF* file, Grid::Ptr grid, std::size_t slice_idx, std::size_t start) {
		...
		template <typename element_t>
		std::vector<element_t> TIFFWriterTemplated<element_t>::read_subpixels_from_slice(std::vector<pixel_t>& src,
		...
		template <typename element_t>
		bool TIFFWriterTemplated<element_t>::write_tiff_strips(TIFF* file, std::vector<element_t>& data, std::size_t width,
		  std::size_t height, std::uint32_t estimated_strip_size) {
			// check parameters are valid :
			if (file == NULL || file == nullptr) {
				std::cerr << "Tried to write to NULL.\n";
				return false;
			}
			if (data.size() == 0) {
				std::cerr << "Tried to write no data.\n";
				return false;
			}

			// ADDITIONAL CHECK : see if data has a round multiple of 'width' elements :
			if ((data.size() % width) != 0) {
				std::cerr << "ERROR : too many components (over=" << data.size() % width << ")\n";
			}

			// pre-allocate a copy buffer for libtiff :
			void* tiff_buffer = _TIFFmalloc(width * sizeof(pixel_t));
			if (tiff_buffer == NULL) {
				std::cerr << "couldn't allocate tiff buffer";
				return false;
			}
			...
			for (std::size_t current_row = 0; current_row < height; current_row++) {
				...
				if (TIFFWriteScanline(file, tiff_buffer, current_row) < 0) {
					std::cerr << "ERROR could not write the contents of the row " << current_row << " of the file \"" << TIFFFileName(file) << "\"\n";
