...
namespace Image {
	...
	namespace Tiff {
		...
		TIFFWriter::TIFFWriter(std::string bname) :
			GenericGridWriter(bname) {}

		TIFFWriter::TIFFWriter(std::string bname, std::string bpath) :
			GenericGridWriter(bname, bpath) {}
		...
		std::string TIFFWriter::build_iterative_filename(std::size_t slice_idx, std::size_t channel) {
			...
			if (this->file_base_name.empty()) {
				name = "default_grid";
			}
			if (this->file_base_path.empty()) {
				path = "/";
			}

			// compute number suffixes :
			if (channel < 10) {
				channel_string = "0" + std::to_string(channel);
			}
			...
			if (slice_idx == 0) {
				leading_0_slice = 4;
			}	 // special case for 0 since log10(0) == -inf
			std::string number_prefix = "";
			if (leading_0_slice > 0) {
				number_prefix = std::string(leading_0_slice, '0');
			}
			slice_string = number_prefix + std::to_string(slice_idx);

			final_name = path;	  // start with base path
			if (final_name[final_name.size() - 1] != '/') {
				final_name += "/";
			}	 // add path separator at the end, if needed
			final_name += name;	   // add file name
			if (final_name[final_name.size() - 1] != '_') {
				final_name += "_";
			}	 // add underscore for numbering if not
