		...
		TIFFReader::Ptr createBackend(const std::string reference_filename) {
			...
			switch (sf) {
				...
				case SAMPLEFORMAT_UINT: {
					if (bps == 8) {
						pImpl = TIFFReaderTemplated<std::uint8_t>::createBackend(w, h, _dim);
						return pImpl;
					}
					if (bps == 16) {
						pImpl = TIFFReaderTemplated<std::uint16_t>::createBackend(w, h, _dim);
						return pImpl;
					}
					if (bps == 32) {
						pImpl = TIFFReaderTemplated<std::uint32_t>::createBackend(w, h, _dim);
						return pImpl;
					}
					if (bps == 64) {
						pImpl = TIFFReaderTemplated<std::uint64_t>::createBackend(w, h, _dim);
						return pImpl;
					}
				...
				case SAMPLEFORMAT_INT: {
					if (bps == 8) {
						pImpl = TIFFReaderTemplated<std::int8_t>::createBackend(w, h, _dim);
						return pImpl;
					}
					if (bps == 16) {
						pImpl = TIFFReaderTemplated<std::int16_t>::createBackend(w, h, _dim);
						return pImpl;
					}
					if (bps == 32) {
						pImpl = TIFFReaderTemplated<std::int32_t>::createBackend(w, h, _dim);
						return pImpl;
					}
					if (bps == 64) {
						pImpl = TIFFReaderTemplated<std::int64_t>::createBackend(w, h, _dim);
						return pImpl;
					}
				...
				case SAMPLEFORMAT_IEEEFP: {
					if (bps == 32) {
						pImpl = TIFFReaderTemplated<float>::createBackend(w, h, _dim);
						return pImpl;
					}
					if (bps == 64) {
						pImpl = TIFFReaderTemplated<double>::createBackend(w, h, _dim);
						return pImpl;
					}
