...
namespace Image {
	...
	namespace Tiff {
		...
		static void extractInfoFromFirstImageOMETIFF(const std::string reference_filename, uint16_t& bps, uint32_t& w, uint32_t& h, uint16_t& sf, uint32_t& z, glm::vec3& voxel_dim) {
			...
			if (strlen(desc) == 0) {
				throw std::runtime_error("Error: the first image used for the specification of the OME TIFF format did not contain any description");
			}
		...
		TIFFReader::Ptr createBackend(const std::string reference_filename, bool isOMETIFF) {
			...
			switch (sf) {
				...
				case SAMPLEFORMAT_UINT: {
					if (bps == 8) {
						pImpl = TIFFReaderTemplated<std::uint8_t>::createBackend(w, h, _dim, z, voxel_dim);
						return pImpl;
					}
					if (bps == 16) {
						pImpl = TIFFReaderTemplated<std::uint16_t>::createBackend(w, h, _dim, z, voxel_dim);
						return pImpl;
					}
					if (bps == 32) {
						pImpl = TIFFReaderTemplated<std::uint32_t>::createBackend(w, h, _dim, z, voxel_dim);
						return pImpl;
					}
					if (bps == 64) {
						pImpl = TIFFReaderTemplated<std::uint64_t>::createBackend(w, h, _dim, z, voxel_dim);
						return pImpl;
					}
				...
				case SAMPLEFORMAT_INT: {
					if (bps == 8) {
						pImpl = TIFFReaderTemplated<std::int8_t>::createBackend(w, h, _dim, z, voxel_dim);
						return pImpl;
					}
					if (bps == 16) {
						pImpl = TIFFReaderTemplated<std::int16_t>::createBackend(w, h, _dim, z, voxel_dim);
						return pImpl;
					}
					if (bps == 32) {
						pImpl = TIFFReaderTemplated<std::int32_t>::createBackend(w, h, _dim, z, voxel_dim);
						return pImpl;
					}
					if (bps == 64) {
						pImpl = TIFFReaderTemplated<std::int64_t>::createBackend(w, h, _dim, z, voxel_dim);
						return pImpl;
					}
				...
				case SAMPLEFORMAT_IEEEFP: {
					if (bps == 32) {
						pImpl = TIFFReaderTemplated<float>::createBackend(w, h, _dim, z, voxel_dim);
						return pImpl;
					}
					if (bps == 64) {
						pImpl = TIFFReaderTemplated<double>::createBackend(w, h, _dim, z, voxel_dim);
						return pImpl;
					}
					std::string err = "Sample was floating point , but no matching ctor was found for " + std::to_string(bps) + " bits.";
					throw std::runtime_error(err);
				} break;
