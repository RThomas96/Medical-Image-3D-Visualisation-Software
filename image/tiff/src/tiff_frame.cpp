...
namespace Tiff {
	...
	uint32_t Frame::height(TIFF* fhandle) const {
		bool shouldClose = false;
		if (fhandle == nullptr) { shouldClose = true; fhandle = this->getLibraryHandle(); }
		uint32_t height = 0;
	...
	uint16_t Frame::sampleFormat(TIFF *fhandle) const {
		bool shouldClose = false;
		if (fhandle == nullptr) { fhandle = this->getLibraryHandle(); shouldClose = true; }
	...
	void Frame::loadTIFFInfo(std::string_view fname) {
		...
		if (this->sourceFile.empty() == false) {
			...
			if (TIFFIsTiled(file)) { TIFFClose(file); throw std::runtime_error("Cannot read tiled images yet"); }
			...
			if (result != 1) { TIFFClose(file); throw std::runtime_error("Cannot get PlanarConfig. Image is malformed."); }
			if (pconfig != 1) {
				TIFFClose(file);
			...
			if (result != 1) { TIFFClose(file); throw std::runtime_error("Cannot read SamplesPerPixel."); }
