...
vec4 R8UIToRGB(in uvec3 ucolor) {
	...
	if (color_r < textureBounds.x || color_r > textureBounds.y) { return planeIndexToColor(); }
...
vec4 R8UItoColorScale(in uvec3 ucolor) {
	if (ucolor.r < colorBounds.x) { return vec4(.0, .0, .0, 1.); }
	if (ucolor.r > colorBounds.y) { return vec4(1., 1., 1., 1.); }
...
vec4 planeIndexToColor() {
	if (currentPlane == 1) { return vec4(1., .0, .0, .2); }
	if (currentPlane == 2) { return vec4(.0, 1., .0, .2); }
	if (currentPlane == 3) { return vec4(.0, .0, 1., .2); }
