...
void main(void)
{
	...
	vec4 pp4 = planePositions.xyzx; pp4.w = .0;
	vec4 gp4 = gridPosition.xyzx; gp4.w = .0;
	vec4 threshold = gp4 + pp4;
	if (vPos_WS.x < threshold.x) { discard; } // Early discards for
	if (vPos_WS.y < threshold.y) { discard; } // the parts of the grid
	if (vPos_WS.z < threshold.z) { discard; } // behind the planes
