...
namespace Image {
	...
	public:
		...
		ReadCache(void) :
			m_data(0), lastInsertedElement(0) {}
		...
		data_t_ptr getDataIndexed(const std::size_t idx) {
			// Check if we have the data, and if we do return it immediately :
			if (idx >= this->maxCachedElements || idx >= this->m_data.size()) {
				return nullptr;
			}
			// Otherwise, return a nullptr :
		...
		std::size_t loadData(const index_t index, data_t_ptr& data) {
			...
			if (this->m_data.size() == this->maxCachedElements) {
				...
				if (this->lastInsertedElement == this->maxCachedElements - 1) {
		...
		void clearCache(void) {
			...
			for (std::pair<index_t, data_t_ptr>& cached : this->m_data) {
				cached.second.reset();
			}
			// Clear the vector :
			this->m_data.clear();
