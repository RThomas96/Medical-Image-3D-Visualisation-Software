...
namespace Image {
	...
	GridWriter::GridWriter(GenericGridWriter::Ptr _existing_backend) :
		pImpl(std::move(_existing_backend)) {}
	...
	void GridWriter::setBaseName(std::string basename) {
		...
		if (this->pImpl) {
			this->pImpl->setBaseName(basename);
		}
		return;
	}
	...
	void GridWriter::setBasePath(std::string basepath) {
		...
		if (this->pImpl) {
			this->pImpl->setBasePath(basepath);
		}
		return;
	}
	...
	bool GridWriter::writeSlice(Grid::Ptr src_grid, std::size_t slice, ThreadedTask::Ptr &task) {
		if (this->pImpl) {
			return this->pImpl->writeSlice(src_grid, slice, task);
		}
		return false;
	}
	...
	bool GridWriter::writeGrid(Grid::Ptr src_grid, ThreadedTask::Ptr &task) {
		if (this->pImpl) {
			return this->pImpl->writeGrid(src_grid, task);
		}
		return false;
	}
