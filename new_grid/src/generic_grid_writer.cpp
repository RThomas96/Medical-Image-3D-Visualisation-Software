...
namespace Image {
	...
	GenericGridWriter::GenericGridWriter(std::string bname) :
		file_base_name(bname), file_base_path("/") {}

	GenericGridWriter::GenericGridWriter(std::string bname, std::string bpath) :
		file_base_name(bname), file_base_path(bpath) {}

	void GenericGridWriter::setBaseName(std::string basename) {
		this->file_base_name = basename;
		return;
	}
	void GenericGridWriter::setBasePath(std::string basepath) {
		this->file_base_path = basepath;
		return;
	}
